...
public class BOJ_거짓말 {
	...
	public static void main(String[] args) throws IOException {
		...
		// 2. 진실을 아는 사람 정보 받아오기 truePeople[진실을 아는사람] = true
		st = new StringTokenizer(br.readLine());
		int n = Integer.parseInt(st.nextToken()); // 두번째 줄 = 진실을 아는 사람의 수
		for(int i=0; i<n; i++) {
		...
		// 5. 진실을 아는 사람들과 파티를 같이하지 않았으면 total++
		for(int i=0; i<M; i++) {
			if(peoples[i].size() > 0) {
