...
public class 마법사상어와토네이도 {
	static int N;
	static int[][] map;
	static int[] dx = {0,1,0,-1}; // 토네이도의 x이동 방향
	static int[] dy = {-1,0,1,0}; // 토네이도의 y이동 방향
	static int[] dc = {1,1,2,2}; // 토네이도의 각 방향으로 이동하는 횟수
	static int[][] dsx = {{-1,1,-1,-1,1,2,-1,1,0}, {-1,-1,0,0,0,0,1,1,2}, {1,-1,2,1,-1,-2,1,-1,0},{1,1,0,0,0,0,-1,-1,-2}}; // 모래가 퍼지는 x 방향
	...
	public static void main(String[] args) throws IOException{
		...
		int result = calculateOutSand(N/2, N/2); // 시작점인(map의 중앙 좌표)
		System.out.println(result);
		
	}
	// 현재위치에서 이동 -> 이동한 위치의 모래 뿌리기 -> 이동한 위치를 현재위치로 업데이트
