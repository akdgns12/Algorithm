...
public class 복습2_원판돌리기 {
	...
	public static void main(String[] args) throws IOException{
		...
		for(int i=0; i<T; i++) {
			st = new StringTokenizer(br.readLine());
			int x = Integer.parseInt(st.nextToken()); // x의 배수 번호 원들 회전
			int d = Integer.parseInt(st.nextToken()); // 방향
			int k = Integer.parseInt(st.nextToken()); // 움직인 칸 수
			...
			// 인접한 곳에 같은 숫자가 있는지 확인해주고
			// 없다면 문제에서 나온 조건대로 평균값 계산
			checkPlate();
		}
