...
/*
 * 구현
 * shark클래스를 선언해서 상어의 좌표, 크기, 속력, 방향을 저장하고.
 * 이 상어들을 모아둔 shark HashMap 자료구조 사용
 * 
 * 상어의 크기는 중복되지 않는다는 조건이 있기 때문에 키값은 상어의 크기로 지정
 * 
 * 모든 상어를 이동한 후에 크기를 비교하도록 temp배열을 따로 선언해서 계산한 후 다시
 * arr배열에 덮어쓰는 방법 사용
 * 
 * 상어의 이동은 두가지 방법이 있는데 하나는 좌표를 직접 연산해서 한번에 구하는거고
 * 다른 하나는 반복문으로 일일히 이동하는 것.
 * 
 */
public class 낚시왕 {
